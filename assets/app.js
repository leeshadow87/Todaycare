/* =====================================================
   오늘아이돌봄지도 V14 - app.js
   업체등록 전환 + 사설 업체 기본정보 선등록/수정요청 흐름
   카드 정렬: 실제 업체등록(0) > 공공/기본정보(1) > 외부확인(2)
   ===================================================== */
...
// 지도 정책:
// 카카오맵 SDK는 지도 배경 렌더링과 자체 DB 위도/경도 마커 표시용으로만 사용합니다.
// 카카오 Local/Places API 검색 결과를 장소 DB로 저장, 복제, 재배포하지 않습니다.
// 공개 지도 SDK appkey는 카카오 개발자 콘솔에서 허용 도메인을 반드시 제한해야 합니다.
...
const CAT_EMOJI = {
  support:'👨‍👩‍👧', public:'🎪', childcare:'🏠', schoolcare:'📚', playground:'🌳'
};
const CAT_CLASS = {
  support:'tag-support', public:'tag-public',
  childcare:'tag-childcare', schoolcare:'tag-schoolcare', playground:'tag-playground'
};
...
const DEFAULT_POS  = { lat: 36.6587, lng: 126.6728 }; // 충남도청·내포신도시 중심
...
const PRIORITY_NEIGHBORHOODS = Object.freeze([
  { key: '홍성군', name: '홍성군', label: '홍성군', sub: '충청남도' },
  { key: '예산군', name: '예산군', label: '예산군', sub: '충청남도' },
  { key: '동탄구', name: '화성시 동탄구', label: '화성시 동탄구', sub: '경기도 화성시' }
]);
...
function sourceRank(p) {
  // 0: 실제 업체 직접 등록  1: 공식·공개 DB  2: 외부 링크형
  const s = [
    p.data_source_type, p.verification_status, p.source_name, p.source_url
  ].map(v => String(v || '').toLowerCase()).join('|');
  if (p.operator_registered === true || p.operatorSubmitted === true || p.operator_submitted === true || p.sourceType === 'operator') return 0;
...
function sourceLabel(p) {
  ...
  if (p.sourceType === 'verified_sample' || p.verification_status === 'sample') return { text:'검수샘플', cls:'public' };
  if (p.data_source_type === 'public_lifestyle') return { text:'생활놀이', cls:'public' };
  if (p.data_source_type === 'public_basic' || p.data_source_type === 'owner_outreach_seed') return { text:'기본정보', cls:'public' };
  if (r === 2) return { text:'외부확인', cls:'link' };
  return { text:'공공DB', cls:'public' };
}
function feeLabel(f) {
...
function renderDetail(p) {
  ...
  const noticeTexts = [
    ...
    '이 정보는 공공데이터·공식 공개 자료·기본 사실 정보 기반입니다. 실제 운영 여부는 기관에 확인하세요.',
  ...
  $('detail-body').innerHTML = `
    ...
    <div class="legal-box">
      오늘아이돌봄지도는 시설 정보를 연결하는 서비스이며, 직접 돌봄을 제공하거나 예약·당일 이용을 보증하지 않습니다.
      표시된 기본정보의 수정·삭제가 필요한 소유자/운영자는 아래 요청 버튼으로 연락할 수 있습니다.
    </div>
    <div class="detail-actions">
      ...
      <button class="det-btn" onclick="requestOwnerCorrection('${esc(p.id)}')">정보 수정·삭제 요청</button>
      <button class="det-btn" onclick="sharePlace('${esc(p.id)}')">공유</button>
    </div>`;

  $('detail-panel').classList.add('open');
  document.body.classList.toggle('detail-open-mobile', window.innerWidth <= 640);
}
...
function filterSuggestions(q) {
  ...
  if (!lower) {
    const priorityHtml = PRIORITY_NEIGHBORHOODS.map(h =>
      `<div class="loc-item loc-priority" onclick="setNeighborhood('${h.key}','${h.name}')">
        <span>📍 ${h.label}</span>
        <span class="loc-sub-text">${h.sub}</span>
      </div>`
    ).join('');

    // 기본: 우선 지역 + 시도 전체 목록
    box.innerHTML = priorityHtml + SIDO_LIST.map(sido =>
...
function buildDataHtml(lower) {
  ...
  PRIORITY_NEIGHBORHOODS
    ...
    .forEach(h => {
      html += `<div class="loc-item loc-priority" onclick="setNeighborhood('${h.key}','${h.name}')">
        <span>📍 ${hl(h.label)}</span>
        <span class="loc-sub-text">${h.sub}</span>
      </div>`;
    });
...
function appendKakaoDong(q, lower, box) {
  if (!hasKakaoMaps() || !kakao.maps.services) return;
...
function moveMapByKeyword(name) {
  if (!map || !name || !hasKakaoMaps() || !kakao.maps.services) return;
...
function openVendorGoogleForm() {
  ...
  toast('구글폼을 열었습니다. 등록 후 1~3일 내 검수합니다 🙏');
}
function openVendorModal() { $('vendor-modal').classList.add('open'); }

function requestOwnerCorrection(id) {
  const p = ALL_DATA.find(x => x.id === id);
  if (!p) return;
  const subject = `[오늘아이돌봄] 정보 수정/삭제 요청 - ${p.name}`;
  const body = [
    '안녕하세요. 오늘아이돌봄지도 정보 수정/삭제 요청입니다.',
    '',
    `업체/기관명: ${p.name}`,
    `현재 주소: ${p.address || ''}`,
    `현재 전화: ${p.tel || ''}`,
    '',
    '요청 내용:',
    '- 수정 요청 / 삭제 요청 중 선택해 작성해주세요.',
    '- 운영자 또는 관계자 확인을 위해 연락 가능한 정보를 남겨주세요.',
    '',
    '감사합니다.'
  ].join('\n');
  window.location.href = `mailto:${VENDOR_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

function openInitialDeepLink() {
  const params = new URLSearchParams(window.location.search);
  if (params.get('vendor') === 'register') {
    setTimeout(openVendorModal, 350);
  }
}
...
function closeModal(id) {
  ...
  if (id === 'legal-modal') scheduleInstallPrompt();
}

// ── 법적 고지 (1회) ──────────────────────────────────
function confirmLegal() {
  lsSet('legal_v13_confirmed', '1');
  closeModal('legal-modal');
}
function checkLegal() {
  if (lsGet('legal_v13_confirmed') === '1' || lsGet('legal_v11_confirmed') === '1') return;
...
function initMap() {
  if (!hasKakaoMaps()) {
    $('kakao-map').innerHTML = '<div class="map-fallback">지도를 불러오지 못했습니다.<br><small>배포 도메인을 카카오 개발자 콘솔에 등록한 뒤 다시 확인해주세요.</small></div>';
    return;
  }
...
function handleResize() {
  ...
  const fabGroup = $('mobile-fab-group');
  if (isMobile) {
...
async function init() {
  ...
  try {
    ...
    $('loading').innerHTML = `<div style="color:var(--brand);font-size:13px">
      ❌ 데이터를 불러오지 못했습니다.<br>
      <small>배포 시 <b>data/places.json</b> 경로를 확인하고, 로컬 실행 시에는 Live Server를 사용해주세요.</small>
    </div>`;
    return;
  }
...
// Android Chrome: 설치 가능 시점에 이벤트 포착
...
window.addEventListener('appinstalled', () => {
  const b = $('pwa-install-banner');
  if (b) b.classList.remove('show');
  toast('앱 설치 완료! 홈 화면에서 바로 열 수 있어요 🎉');
...
function isDismissedRecently(key) {
  const until = Number(lsGet(key) || 0);
  return until && Date.now() < until;
}

function scheduleInstallPrompt() {
  if (isStandaloneApp() || hasActiveModal()) return;
  if (_deferredInstallPrompt && !isDismissedRecently('pwa_install_dismissed_until')) {
    setTimeout(() => showBanner('pwa-install-banner'), 2500);
    return;
  }
  checkAndroidInstallHelp();
  checkIosInstall();
}
...
function dismissPwaInstall() {
  const b = $('pwa-install-banner');
  if (b) b.classList.remove('show');
  lsSet('pwa_install_dismissed_until', String(Date.now() + INSTALL_DISMISS_COOLDOWN_MS));
}
...
function checkAndroidInstallHelp() {
  const ua = navigator.userAgent;
  const isAndroid = /android/i.test(ua);
  const isKnownInApp = /kakaotalk|naver|instagram|fbav|line\//i.test(ua);
  if (!isAndroid || isStandaloneApp() || isDismissedRecently('pwa_install_dismissed_until')) return;
  setTimeout(() => {
    if (_deferredInstallPrompt || hasActiveModal()) return;
    const b = $('pwa-install-banner');
    if (!b) return;
    const title = b.querySelector('.pwa-banner-text b');
    const desc = b.querySelector('.pwa-banner-text span');
    const btn = b.querySelector('.pwa-banner-btn');
    if (title) title.textContent = isKnownInApp ? 'Chrome에서 설치하기' : '앱 설치가 안 보이나요?';
    if (desc) desc.textContent = isKnownInApp ? '인앱브라우저는 설치를 막을 수 있어요' : '설치 버튼이 안 뜨면 방법을 확인하세요';
    if (btn) btn.textContent = '방법 보기';
    b.classList.add('show');
  }, 4500);
}

// iOS Safari: beforeinstallprompt 미지원 → 안내 배너
function checkIosInstall() {
  const isIos = /iphone|ipad|ipod/i.test(navigator.userAgent);
  const isSafari = /^((?!crios|fxios|edgios|naver|kakaotalk).)*safari/i.test(navigator.userAgent);
  if (isIos && isSafari && !isStandaloneApp() && !isDismissedRecently('pwa_ios_dismissed_until')) {
    setTimeout(() => showBanner('pwa-ios-banner'), 2500);
  }
}

function dismissIosBanner() {
  const b = $('pwa-ios-banner');
  if (b) b.classList.remove('show');
  lsSet('pwa_ios_dismissed_until', String(Date.now() + INSTALL_DISMISS_COOLDOWN_MS));
}
